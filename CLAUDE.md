...
This is the Gemini CLI, a command-line AI workflow tool. The project is structured as a monorepo using npm workspaces with two main packages:

1. **CLI package (`packages/cli`):** User-facing portion handling input, UI, themes, and commands
2. **Core package (`packages/core`):** Backend that manages Gemini API interactions and tool execution

## Architecture

- **Tools** are located in `packages/core/src/tools/` and extend Gemini's capabilities (file system, shell, web fetch, etc.)
- **Commands** are located in `packages/cli/src/ui/commands/` and handle user-facing CLI commands
- Uses **Ink** (React for CLI) for the terminal UI
- Follows a modular architecture with clear separation between frontend (CLI) and backend (Core)
...
```bash
# From package directory (e.g., packages/cli)
...
The project includes an OpenAI adapter that allows using OpenAI-compatible APIs (including custom providers like Moonshot AI/Kimi). Configuration is done through:
...
### Model Mapping (settings.json)
Custom model mappings can be configured in settings.json to map Gemini model names to OpenAI-compatible model names:

```json
{
  "selectedAuthType": "openai-api-key",
  "openaiModelMapping": {
    "gemini-2.5-pro": "custom-model-name",
    "gemini-2.5-flash": "another-custom-model"
  }
}
```

The adapter will use these mappings when translating model names from Gemini to the OpenAI-compatible API.

## Important Workflow Notes

### Integration Tests
- Located in `integration-tests/` directory
- Can be run with sandboxing enabled (Docker, Podman, or macOS Seatbelt)
- Use `npm run test:e2e` for basic e2e tests
- Use `npm run test:integration:all` to test with different sandbox providers

### VS Code Extension
- A VS Code companion extension exists in `packages/vscode-ide-companion`
- Build with `npm run build:vscode`
